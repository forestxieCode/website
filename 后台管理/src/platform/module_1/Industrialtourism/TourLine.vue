        ...
        <el-col :span="24" >
             <el-table v-loading="loading" :data="tableData" :row-style="HeightClass"  :row-class-name="tableRowClassName" border style="width: 100%">
                   <el-table-column
                    ...
                    >
                      <template slot-scope="scope">
                             {{scope.row.childType==1?'金牌活动':'品牌介绍'}}
                      </template>
                    </el-table-column>
...
import { getList_ , isPublish ,deleteNews  } from '@/api/index'
...
export default {
  ...
  methods: {
    ...
    go_to_add(){
      this.$router.push({path:'/Home/AddWalkIntoInfo',query:{type:4}});
    },
    go_to_eidt(id){
      this.$router.push({path:'/Home/AddWalkIntoInfo',query:{id:id,type:4}});
    },
    ...
    getList(){
      ...
      getList_(this.parameter).then(res =>{
        if(res.success){
          this.value6=''
